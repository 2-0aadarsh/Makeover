...
import { formatAmount, parseDurationToMinutes } from "../../../utils/paymentUtils";
...
const Checkout = ({
  ...
  const handlePaymentSubmit = async () => {
    ...
    try {
      ...
      if (selectedDate && selectedSlot) {
        ...
        if (bookingDateOnly.getTime() === today.getTime()) {
          ...
          console.log("🔍 Time slot validation for today:", {
            ...
            isBookingTimeValid: bookingDateTime >= oneHourFromNow,
          });

          if (bookingDateTime < oneHourFromNow) {
            throw new Error(
              "For today's booking, please select a time slot that is at least 1 hour in the future."
            );
          }
      ...
      // Convert duration to numeric minutes (handles "1hr 10mins" from backend)
      ...
      // Calculate total duration for booking (duration is already numeric from parseDurationToMinutes)
      ...
      if (paymentMethod === "online") {
        ...
        // Only proceed if payment was successful (fulfilled + has success data)
        const isFulfilled = result?.meta?.requestStatus === "fulfilled";
        const backendPayload = result?.payload?.data || result?.payload;
        const hasSuccessData = backendPayload && (backendPayload.orderId || backendPayload.paymentId);

        if (isFulfilled && hasSuccessData) {
          ...
          console.warn("Payment failed – staying on checkout. Error:", result?.payload);
        }
      } else {
        ...
        // Only proceed if order was created successfully (fulfilled + has success data)
        const isFulfilled = result?.meta?.requestStatus === "fulfilled";
        const backendPayload = result?.payload?.data || result?.payload;
        const hasSuccessData = backendPayload && (backendPayload.orderId || backendPayload.bookingId);
        ...
        if (isFulfilled && hasSuccessData) {
          ...
          console.warn("COD order failed – staying on checkout. Error:", result?.payload);
        }
      }
    } catch (error) {
