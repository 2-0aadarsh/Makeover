...
const FlexCard = ({ item, source = "other", onEnquiryClick = null }) => {
  ...
  const handleEnquiryClick = () => {
    ...
    // If parent provides onEnquiryClick, use that (for modal transition)
    // Otherwise use internal modal state (backward compatible)
    if (onEnquiryClick) {
      onEnquiryClick(serviceData, source);
    } else {
  ...
  return (
    <>
      ...
      {/* Enquiry Modal - Only render if parent doesn't handle it */}
      {!onEnquiryClick && (
        <EnquiryModal
