...
const CompletePaymentModal = ({
  ...
  useEffect(() => {
    if (isOpen && booking) {
      ...
      setPaymentMethod('');
      console.log('💳 Modal opened - payment method reset to empty, user must select');
    }
  }, [isOpen, booking]);
  ...
  const handleConfirm = async () => {
    ...
    console.log('💳 Complete payment modal: Confirming with method:', paymentMethod);
    ...
    if (paymentMethod === 'online') {
      console.log('💳 Online payment selected - initiating Razorpay for existing booking');
      ...
      try {
        ...
        const orderData = {
          ...
          customer: {
            name: user?.name || booking.userId?.name || 'Customer',
            email: user?.email || booking.userId?.email || '',
            phone: user?.phoneNumber || booking.bookingDetails?.address?.phone || ''
          },
          bookingDetails: {
        ...
        console.log('💳 Creating Razorpay order for existing booking:', orderData);
  ...
  const handleClose = () => {
    // Reset form
    setPaymentMethod(booking?.paymentDetails?.paymentMethod || '');
    onClose();
  };

  // Handle payment method change
  const handlePaymentMethodChange = (method) => {
    console.log('💳 Complete payment modal: Payment method changed to:', method);
        ...
        <div className="bg-gradient-to-r from-blue-500 to-blue-600 text-white p-6 rounded-t-2xl">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-blue-100 mt-1 text-sm">Choose your payment method</p>
            </div>
            <button
              onClick={handleClose}
              disabled={isLoading}
          ...
          <div className="bg-gray-50 rounded-lg p-4 mb-6">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className={`font-semibold ${paymentStatus === 'pending' ? 'text-yellow-600' : 'text-green-600'}`}>
          ...
          <div className="mb-6">
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
              <div className="flex items-start">
                ...
                <div className="ml-3">
                  <h4 className="text-sm font-semibold text-blue-900 mb-1">Current Payment Method</h4>
                  <p className="text-sm text-blue-700">
                    {currentPaymentMethod === 'online' ? 'Pay Online (UPI, Cards, Wallets)' : 'Pay After Service (Cash on Delivery)'}
                  </p>
                </div>
              </div>
            </div>
              ...
              {paymentMethod === 'online' && (
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  ...
                  <p className="text-sm text-blue-700 mb-2">
                    You will be redirected to Razorpay to complete your payment securely with UPI, Credit/Debit Cards, Net Banking, Wallets, and more.
                  </p>
              ...
              {paymentMethod === 'cod' && (
                <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                  ...
                  <p className="text-sm text-green-700 mb-2">
                    You can pay cash directly to our beautician after service completion.
                  </p>
            ...
            >
              {(isLoading || isProcessingPayment) ? (
                <>
                  ...
                  {isProcessingPayment ? 'Opening Razorpay...' : 'Processing...'}
                </>
              ) : paymentMethod === 'online' ? (
