...
const ServiceModal = ({
  ...
  // Enquiry Modal State for smooth transition
  const [showServiceContent, setShowServiceContent] = useState(true);
  const [showEnquiryModal, setShowEnquiryModal] = useState(false);
  const [enquiryServiceData, setEnquiryServiceData] = useState(null);
  const [enquirySource, setEnquirySource] = useState("other");
  ...
  // Handle enquiry button click from FlexCard
  const handleEnquiryClick = (serviceData, source) => {
    console.log("ServiceModal: Starting enquiry transition for:", serviceData);
    // Store the service data and source
    setEnquiryServiceData(serviceData);
    setEnquirySource(source);
    // Hide service content to trigger exit animation
    setShowServiceContent(false);
    // After service modal exit animation completes, show enquiry modal
    setTimeout(() => {
  ...
  const handleEnquiryClose = () => {
    ...
    // After enquiry modal closes, close the entire modal
    setTimeout(() => {
      onClose();
    }, 300); // Match the enquiry modal exit animation duration
  };
  ...
  // Check if desktop on mount and resize (1280px = tablet gets slide-up like mobile)
  useEffect(() => {
    const checkDesktop = () => {
  ...
  return (
    <>
      <AnimatePresence mode="wait">
        {showServiceContent && (
          <>
            ...
            <motion.div
              key="modal-overlay"
              className="fixed inset-0 z-50 flex items-end xl:items-center justify-center p-0 xl:p-4 bg-black/30 backdrop-blur-sm"
        ...
        >
          ...
          {/* ======================================== */}
          <div
            className="xl:hidden absolute top-3 left-1/2 transform -translate-x-1/2 w-16 h-1.5 bg-gray-400/60 rounded-full cursor-grab active:cursor-grabbing"
            onTouchStart={handleDragStart}
            onMouseDown={handleDragStart}
          />
                    ...
                    <AnimatePresence>
                      {showDropdown && (
                        <>
                          {/* Backdrop for mobile */}
                          <motion.div
                            className="fixed inset-0 bg-black/30 z-[60] xl:hidden"
                            initial={{ opacity: 0 }}
                            animate={{ opacity: 1 }}
                            exit={{ opacity: 0 }}
          ...
          {/* ======================================== */}
          <div className="xl:hidden absolute bottom-0 left-0 right-0 h-8 bg-[#FFFCFD] to-transparent pointer-events-none" />
        </motion.div>
      </motion.div>
          </>
        )}
      ...
      {/* Enquiry Modal - Renders after service modal closes */}
      <EnquiryModal
        isOpen={showEnquiryModal}
        onClose={handleEnquiryClose}
        serviceData={enquiryServiceData}
