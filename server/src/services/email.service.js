...
/**
 * Send admin onboarding welcome email with onboarding link
 * @param {Object} onboardingData - { email, adminName, creatorName, onboardingLink, expiresInHours }
 * @returns {Promise} Email send result
 */
const sendAdminOnboardingEmail = async (onboardingData) => {
  try {
    const { email, adminName, creatorName, onboardingLink, expiresInHours = 48 } = onboardingData;
    
    console.log('📧 Preparing to send admin onboarding email to:', email);
    
    // Generate HTML template
    const emailHtml = adminOnboardingEmailTemplate({
      onboardingLink,
      adminName,
      creatorName,
      expiresInHours
    ...
    // Mail options
    const mailOptions = {
      from: '"WeMakeover Admin Portal"',
      to: email,
      subject: '🎉 Welcome to WeMakeover Admin Portal - Complete Your Onboarding',
      html: emailHtml,
    };
    
    console.log('📧 Sending admin onboarding email to:', email);
    
    // Send email
    const info = await transporter.sendMail(mailOptions);
    
    console.log('✅ Admin onboarding email sent successfully:', info.messageId);
    
    return info;
  } catch (error) {
    console.error('❌ Error sending admin onboarding email:', error);
    // Don't throw error - we don't want to break admin creation flow if email fails
    return null;
  }
};
